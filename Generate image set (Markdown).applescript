...
if TheGridChoice is false then
	...
	set dialogResult to display dialog �
		"Please enter an image width (in %)" buttons {"Cancel", "OK"} �
...
"
								if TheGridSize is 3 then
									set TheCaptionTmp to TheCaptionTmp & " � **Middle**: " & TheCaption
								end if
							end if
						else
							set TheImageLink to TheImageLink & "[![" & TheImageName & "]] "
							set TheReferences to TheReferences & "[" & TheImageName & "]: " & TheImageURL & " style=\"width:" & TheWidth & "%;\"
"
							set TheCaptionTmp to TheCaptionTmp & " � **Right**: " & TheCaption
...
"
						if InsertCaption then
							set TheCaptionTmp to TheCaptionTmp & " � " & RunCounter & ". " & TheCaption
						end if
					end if
